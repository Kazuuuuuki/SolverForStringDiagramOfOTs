...
def synthesizeFile(args):
    ...
    with open(f"{path}/{filename}") as f:
        ...
        for row in reader: 
            ...
            hierarchicalMap.append([ array.array('f', [ 0 for j in range(hierarchicalCodom)]) for i in range(hierarchicalDom) ])
    ...
    ansMonLP = slp.synthesizeLP(lp, cmat.dom, cmat.codom)
    time2 = (end_time2 - start_time2) + ansMonLP[1]
    opMap = ansMonLP[2]
    ...
    lp = slp.LP([[cmat]])
    exact_ans = slp.solveLP(lp, exact=True)[0]
    error1 = abs(ansMonLP[0]-exact_ans)/exact_ans
    error2 = abs(ansCompLP-exact_ans)/exact_ans
    result = [[time1, time2, time3, time1+time2, time1+time2+time3, error1],
              [time4, error2]
    ]
    print(result)
