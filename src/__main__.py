...
def picturingExSeq():
    ...
    ycomplp = []
    for i in range(7):
        path = f'benchmarks/BChains-{(i+1)*100}-100/ans0.csv'
        with open(path) as f:
            reader = csv.reader(f)
            reader = [list(map(float, row)) for row in reader]
            x.append((i+1)*100)
            # ymonlp.append(reader[0][2])
            ysh.append(reader[0][4])
            ycomplp.append(reader[1][0])
    ...
    # plt.show()  
    plt.legend((psh[0], pcomplp[0]), ("MonLP", "CompLP"), loc=2)
...
def run(args):
    ...
    if (args[1] == 'solveExpSeqcomp'):
        for i in range(7):
            numOfSeqcomp = (i+1)*100+99
            s = f'UChains-{numOfSeqcomp}-10-200'
    ...
    if (args[1] == 'synthesizeExpSeqcomp'):
        for i in range(7):
            numOfSeqcomp = (i+1)*100
            s = f'BChains-{numOfSeqcomp}-100'
            sb.synthesizeFile([0, 'synthesizeFile', s, 1])
    ...
    if (args[1] == 'synthesizeForTable'):
        ...
        # BRoom1 == BRooms-199-100-30-40
            s = 'BRooms-199-100-30-40'
            sb.synthesizeFile([0, 'synthesizeFile', s, i])
        # BRoom2 == BRoomsP-209-100-100
            s = 'BRoomsP-209-100-100'
            sb.synthesizeFile([0, 'synthesizeFile', s, i])
        # URoom1 == URooms-399-10-500-4-240-270-3
